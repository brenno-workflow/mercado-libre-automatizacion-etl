...
def executar_etl_autenticado():
    ...
    #url_busca = "https://api.mercadolibre.com/items/MLA2890756564"
    #url_busca = "https://api.mercadolibre.com/orders/search?seller=2018580197&order.status=paid&limit=1"
    #url_busca = "https://api.mercadolibre.com/users/2018580197"
    #url_busca = "https://api.mercadolibre.com/users/480263032"
    #url_busca = "https://api.mercadolibre.com/users/480263032/items/search"
    #url_busca = "https://api.mercadolibre.com/items/MLA2890756564/visits"
    #url_busca = "https://api.mercadolibre.com/items?ids=MLA2890756564"
    url_busca = "https://api.mercadolibre.com/items/bulk?ids=MLA2890756564"
    ...
    for product_id in product_ids:
        ...
        if response.ok:
            ...
            print(f"Publicações encontradas: {len(results)}")

            for item in results:
                print(
                    f"  Item: {item.get('item_id')} | "
                    f"Seller: {item.get('seller_id')} | "
                    f"Preço: {item.get('price')} {item.get('currency_id')} | "
